(() => {
  ...
  function render(){
    ...
    list.innerHTML=visible.map((f,i)=>{const status=states[f.id]||'review';return `<article class="trust-finding-card severity-${f.severity}" data-finding-id="${f.id}"><div class="finding-head"><span class="finding-rank">${i+1}</span><div><span class="finding-severity">${esc(f.severity)}</span><h4>${esc(f.title)}</h4></div><select class="remediation-status" aria-label="Remediation status for ${esc(f.title)}"><option value="review" ${status==='review'?'selected':''}>To review</option><option value="progress" ${status==='progress'?'selected':''}>In progress</option><option value="resolved" ${status==='resolved'?'selected':''}>Resolved</option><option value="accepted" ${status==='accepted'?'selected':''}>Accepted risk</option></select></div><div class="finding-block verified-block"><b>Verified scanner finding</b><p>${esc(f.evidence)}</p></div><div class="finding-block action-block"><b>Suggested next step</b><p>${esc(actionFor(f.severity))}</p></div></article>`}).join('');
  }
  function snapshot(){ const states=readStatuses(); return {version:'3.49.0',generatedAt:new Date().toISOString(),target:$('targetInput')?.value||'',risk:$('riskLabel')?.textContent?.trim()||'',score:$('scoreValue')?.textContent?.trim()||'',summary:$('summaryText')?.textContent?.trim()||'',findings:findings.map(f=>({...f,status:states[f.id]||'review'})),notes:$('caseNotes')?.value||''}; }
  ...
  $('scanButton')?.addEventListener('click',()=>setTimeout(collect,220));
