(() => {
  ...
  function collectCaseManagement(){
    const live = window.ProxumaCaseManagement?.get?.();
    return live || {
      name:value('caseManagementName'),
      status:value('caseManagementStatus') || 'open',
      priority:value('caseManagementPriority') || 'medium',
      notes:value('caseManagementNotes'),
      finalDecision:value('caseManagementDecision'),
      updatedAt:text('caseManagementUpdated')
    };
  }
  ...
  function snapshot(){
    ...
    return {
      ...
      notes: value('caseManagementNotes') || value('caseNotes') || value('caseJournalNote') || '',
  ...
  function readableText(r){
    ...
    const m=r.caseManagement || {};
    lines.push('', 'CASE MANAGEMENT', `Name: ${m.name || 'Not named'}`, `Status: ${m.status || 'open'}`, `Priority: ${m.priority || 'medium'}`, `Last updated: ${m.updatedAt || 'Not recorded'}`);
    if(m.finalDecision) lines.push(`Final decision: ${m.finalDecision}`);
